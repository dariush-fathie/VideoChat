...
import ir.jin724.videochat.webRTC.utils.AppRTCUtils
...
class AppRTCAudioManager private constructor(context: Context) {
    ...
    private inner class WiredHeadsetReceiver : BroadcastReceiver() {
        ...
        override fun onReceive(
            ...
            // todo do not allow connecting without MIC
            val microphone =
                intent.getIntExtra("microphone", Companion.HAS_NO_MIC)
            ...
            Log.d(
                ...
                "WiredHeadsetReceiver.onReceive" + AppRTCUtils.threadInfo
                        ...
                        + (if (state == Companion.STATE_UNPLUGGED) "unplugged" else "plugged").toString()
                        + ", m=" + (if (microphone == Companion.HAS_MIC) "mic" else "no mic").toString()
                        + ", n=" + name?.toString() + ", sb="
    ...
    private fun setAudioDeviceInternal(device: AudioDevice) {
        ...
        when (device) {
            AudioDevice.SPEAKER_PHONE -> setSpeakerphoneOn(
                true
            )
            AudioDevice.EARPIECE -> setSpeakerphoneOn(false)
            AudioDevice.WIRED_HEADSET -> setSpeakerphoneOn(
                false
            )
    ...
    fun setDefaultAudioDevice(defaultDevice: AudioDevice) {
        ThreadUtils.checkIsOnMainThread()
        when (defaultDevice) {
            AudioDevice.SPEAKER_PHONE -> defaultAudioDevice =
                defaultDevice
